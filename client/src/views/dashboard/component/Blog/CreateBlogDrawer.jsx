...
import { apiCreatePostTag, apiGetTag } from '../../../../services/tag';
import { useSelector } from 'react-redux';
import { apiCreateBlog } from '../../../../services/blog';


const { Option } = Select;


const CreateBlogDrawer = ({ onClose, open }) => {
    ...
    const handleOk = async () => {
        ...
        if (selectedImages.length > 0) {
            ...
            try {
                ...
                const responeBlog = await apiCreateBlog(payloadBlogMedia);
                if (selectedTags?.length) {
                    for (const tag of selectedTags) {
                        const payload = {
                            data: {
                                document_share_id: responeBlog.data.data.documentId,
                                tag_id: tag,
                            }
                        };
                        try {
            ...
            try {
                ...
                const responeBlog = await apiCreateBlog(payload);
                if (selectedTags?.length) {
                    for (const tag of selectedTags) {
                        const payload = {
                            data: {
