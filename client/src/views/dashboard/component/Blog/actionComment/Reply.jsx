...
const Reply = ({ blog, parent, nested, handleReplyFormClose }) => {
  const { profile } = useSelector((state) => state.root.user || {});
  const [comment, setComment] = useState((nested ? `@${nested?.users_id?.username} ` : ''));
  ...
  const handleSubmit = async (e) => {
    ...
    setComment(nested ? `@${nested?.users_id?.username} ` : ''); // Reset the comment
      ...
      <div className="d-flex align-items-center">
        ...
        <form className="align-items-center m-2 replyForm" onSubmit={handleSubmit}>
          ...
          <div className="d-flex align-items-center m-1 replyEmoSend">
            ...
            <Send formData={{
              inputText: comment,
            }} blog={blog} parent={parent} nested={nested} profile={profile} handleClose={handleReplyFormClose} />
          </div>
