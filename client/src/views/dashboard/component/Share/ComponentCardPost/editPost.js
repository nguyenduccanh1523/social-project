...
import { apiGetTag, apiGetPostTag } from "../../../../../services/tag";
import { apiGetFriendAccepted, apiGetPostFriend } from "../../../../../services/friend";
...
const EditPostModal = ({ show, handleClose, post, page, validSources }) => {
    ...
    const [selectedImages, setSelectedImages] = useState(validSources.map(src => ({ url: src })));
    const [showConfirm, setShowConfirm] = useState(false);
    const [visibility, setVisibility] = useState(post?.type_id?.name || 'public');
    ...
    // Handle selected friends change
                ...
                <Modal.Body>
                    <form>
                            ...
                            <li className="col-md-6 mb-3">
                                <div className="bg-soft-primary rounded p-2 pointer me-3" style={{ zIndex: 1050 }}>
                                    <Select
                                        ...
                                        onChange={handleFriendSelect} // Sử dụng handleFriendSelect
                                        value={selectedFriends} // Đảm bảo giá trị của Select là `selectedFriends`
                            ...
                            <li className="col-md-6 mb-3">
                                <div className="bg-soft-primary rounded p-2 pointer me-3" style={{ zIndex: 1050 }}>
                                    <Select
                                        ...
                                        onChange={handleTagSelect} // Sử dụng handleTagSelect
                                        value={selectedTags} // Đảm bảo giá trị của Select là `selectedTags`
                                        getPopupContainer={trigger => trigger.parentNode}
                                        showSearch
                                        filterOption={(input, option) =>
                                            option.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
                        ...
                        {selectedImages.length > 0 && (
                            <div className="d-flex flex-wrap justify-content-center mb-3">
                                {selectedImages.map((image, index) => (
                                    <div key={image.url || index} className="position-relative m-2">
                                        <img
                                            src={image.url}
                                            alt={`Selected ${index}`}
                        ...
                        <ButtonEdit profile={post?.user_id} formData={{
                            ...
                            selectedImages,
                            visibility,
                        }} page={page} post={post} handleClose={handleClose} />
                    </form>
                </Modal.Body>
            </Modal>
            <Modal
