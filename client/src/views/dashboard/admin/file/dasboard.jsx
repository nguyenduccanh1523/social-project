...
import { apiPostMonthly } from '../../../../services/post'
import { apiCommentMonthly, apiReactionMonthly } from '../../../../services/comment'
import { apiUserStatistics } from '../../../../services/user'
import { apiStatisticsYearly } from '../../../../services/search'

const Admin = () => {
    const { token } = useSelector((state) => state.root.auth || {});

    const { data: postMonthly, isLoading: postMonthlylaoding } = useQuery({
        queryKey: ['postMonthly', token],
        queryFn: () => apiPostMonthly({ token }),
        enabled: !!token,
        onSuccess: (data) => {
    ...
    const { data: commentMonthly, isLoading: commentMonthlylaoding } = useQuery({
        queryKey: ['commentMonthly', token],
        queryFn: () => apiCommentMonthly({ token }),
        enabled: !!token,
        onSuccess: (data) => {
            console.log("User data fetched successfully:", data);
        },
    ...
    const { data: reactionMonthly, isLoading: reactionMonthlylaoding } = useQuery({
        queryKey: ['reactionMonthly', token],
        queryFn: () => apiReactionMonthly({ token }),
        enabled: !!token,
        onSuccess: (data) => {
            console.log("User data fetched successfully:", data);
        },
    ...
    const postCount = postMonthly?.data?.data || []
    const commentCount = commentMonthly?.data?.data || []
    const reactionCount = reactionMonthly?.data?.data || []


    const { data: userStatistics, isLoading: userStatisticlaoding } = useQuery({
        queryKey: ['userStatistic', token],
        queryFn: () => apiUserStatistics({ token }),
        enabled: !!token,
        onSuccess: (data) => {
            console.log("User data fetched successfully:", data);
        },
    ...
    const userSta = userStatistics?.data?.data || []

    const { data: StatisticsYearly, isLoading: StatisticsYearlylaoding } = useQuery({
        queryKey: ['StatisticsYearly', token],
        queryFn: () => apiStatisticsYearly({ token }),
        enabled: !!token,
        onSuccess: (data) => {
            console.log("User data fetched successfully:", data);
        },
    ...
    const StatiscYear = StatisticsYearly?.data?.data || []
    console.log('s', StatiscYear)

    // Tạo mảng 12 phần tử, mặc định là 0
    const monthlyData = Array(12).fill(0);
    userSta?.monthlyStats?.forEach(item => {
        if (item.month >= 1 && item.month <= 12) {
            monthlyData[item.month - 1] = item.count;
        }
    });

    // Đảm bảo series luôn hợp lệ cho adminChart
    const adminChart = {
        options: {
            colors: ["#50b5ff"],
            chart: {
        ...
        series: [
            {
                name: 'Total Account',
                data: Array.isArray(monthlyData) && monthlyData.length === 12 ? monthlyData : Array(12).fill(0)
            }
        ],
    }

    // Đảm bảo series luôn hợp lệ cho adminChart1 (radialBar)
    const genderSeries = [
        Number(userSta?.genderStats?.[0]?.percentage) || 0,
        Number(userSta?.genderStats?.[1]?.percentage) || 0
    ];
    console.log('genderSeries', genderSeries);
    ...
    const adminChart1 = {
        ...
        series: Array.isArray(genderSeries) && genderSeries.length === 2 && genderSeries.every(x => typeof x === 'number') ? genderSeries : [0, 0],
    }

    // Đảm bảo series luôn hợp lệ cho adminChart2 (pie)
    const pieSeries = [65, 35]; // Nếu có dữ liệu động thì thay ở đây
    const adminChart2 = {
        options: {
            legend: {
                show: true,
                    ...
                    <Row as="ul" className="list-unstyled mb-0">
                            ...
                            <Card>
                                <Card.Body>
                                    <div className="points">
                                        <span>This Month Posts</span>
                                        <div className="d-flex align-items-center">
                                            <h3>{postCount?.currentCount}</h3>
                                            {postCount?.percentChange !== undefined && (
                                                <small className={`ms-3 ${postCount?.currentCount >= postCount?.prevCount ? "text-success" : "text-danger"}`}>
                                                    {postCount?.currentCount >= postCount?.prevCount ? (
                                                    ...
                                                    {Math.abs(postCount?.percentChange)}%
                                                </small>
                                            )}
                                        </div>
                                    </div>
                        ...
                        <Col md="6" lg="3" as="li">
                            <Card>
                                <Card.Body>
                                    <div className="points">
                                        <span>This Months Posts Liked</span>
                                        <div className="d-flex align-items-center">
                                            <h3>{reactionCount?.currentCount}</h3>
                                            {reactionCount?.percentChange !== undefined && (
                                                <small className={`ms-3 ${reactionCount?.currentCount >= reactionCount?.prevCount ? "text-success" : "text-danger"}`}>
                                                    {reactionCount?.currentCount >= reactionCount?.prevCount ? (
                                                        <span className="material-symbols-outlined">
                                                            trending_up
                                                        </span>
                                                    ) : (
                                                    ...
                                                    {Math.abs(reactionCount?.percentChange)}%
                                                </small>
                                            )}
                                        </div>
                                    </div>
                        ...
                        <Col md="6" lg="3" as="li">
                            <Card>
                                <Card.Body>
                                    <div className="points">
                                        <span>This Month Comments</span>
                                        <div className="d-flex align-items-center">
                                        <h3>{commentCount?.currentCount}</h3>
                                            {commentCount?.percentChange !== undefined && (
                                                <small className={`ms-3 ${commentCount?.currentCount >= commentCount?.prevCount ? "text-success" : "text-danger"}`}>
                                                    {commentCount?.currentCount >= commentCount?.prevCount ? (
                                                        <span className="material-symbols-outlined">
                                                            trending_up
                                                        </span>
                                                    ) : (
                                                    ...
                                                    {Math.abs(commentCount?.percentChange)}%
                                                </small>
                                            )}
                                        </div>
                                    </div>
                                        ...
                                        <Col md="4" lg="4" className="d-grid gap gap-3">
                                            <div className="d-flex align-items-start">
                                                ...
                                                <div className="ms-2" style={{ lineHeight: "1.5" }}>
                                                    ...
                                                    <h6 className="mb-0">{Math.abs(userSta?.genderStats?.[0]?.percentage)}%</h6>
                                                </div>
                                            </div>
                                            <div className="d-flex align-items-start">
                                                ...
                                                <div className="ms-2" style={{ lineHeight: "1.5" }}>
                                                    ...
                                                    <h6 className="mb-0">{Math.abs(userSta?.genderStats?.[1]?.percentage)}%</h6>
                                                </div>
                                            </div>
                                        </Col>
                                ...
                                <Card.Body>
                                    <div className="mb-3">
                                        <div className="d-flex justify-content-between mt-2 text-dark">
                                            <h6>Conversations</h6>
                                            <small>{StatiscYear?.conversations?.percentage}</small>
                                        </div>
                                        <ProgressBar variant="danger" className="mt-2" now={StatiscYear?.conversations?.percentage} style={{ height: "6px" }} />
                                    </div>
                                    <div className="mb-3">
                                        <div className="d-flex justify-content-between mt-2 text-dark">
                                            <h6>Document Shares</h6>
                                            <small>{StatiscYear?.documents?.percentage}</small>
                                        </div>
                                        <ProgressBar variant="info" className="mt-2" now={StatiscYear?.documents?.percentage} style={{ height: "6px" }} />
                                    </div>
                                    <div className="mb-3">
                                        <div className="d-flex justify-content-between mt-2 text-dark">
                                            <h6>Events</h6>
                                            <small>{StatiscYear?.events?.percentage}</small>
                                        </div>
                                        <ProgressBar variant="primary" className="mt-2" now={StatiscYear?.events?.percentage} style={{ height: "6px" }} />
                                    </div>
                                    <div className="mb-3">
                                        <div className="d-flex justify-content-between mt-2 text-dark">
                                            <h6>Groups</h6>
                                            <small>{StatiscYear?.groups?.percentage}</small>
                                        </div>
                                        <ProgressBar variant="success" className="mt-2" now={StatiscYear?.groups?.percentage} style={{ height: "6px" }} />
                                    </div>
                                    <div className="">
                                        <div className="d-flex justify-content-between mt-2 text-dark">
                                            <h6>Livestream hosting</h6>
                                            <small>{StatiscYear?.livestreams?.percentage}</small>
                                        </div>
                                        <ProgressBar variant="warning" className="mt-2" now={StatiscYear?.livestreams?.percentage} style={{ height: "6px" }} />
                                    </div>
                                    <div className="">
                                        <div className="d-flex justify-content-between mt-2 text-dark">
                                            <h6>Pages</h6>
                                            <small>{StatiscYear?.pages?.percentage}</small>
                                        </div>
                                        <ProgressBar variant="dark" className="mt-2" now={StatiscYear?.pages?.percentage} style={{ height: "6px" }} />
                                    </div>
                                </Card.Body>
                            </Card>
                        </Col>
